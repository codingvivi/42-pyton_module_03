...
def main() -> None:
    ...
    names_original: list[str] = list(set.intersection(set(names_base), set(names_title)))
    print(f"New list of capitalized names only: {names_original}")
    print("")

    scores: dict[str, int] = {name: random.randint(0, 1000) for name in names_title}
    print(f"Score dict: {scores}")

    avg: float = sum(s for s in scores.values()) / len(names_title)
    print(f"Score average is {round(avg, 2)}")

    high_scores: dict[str, int] = {name: score for name, score in scores.items() if score > avg}
